...
def absolute_adaptive_gain(read_data_array):
...
def adaptive_gain(read_data_array, higher_is_better):

    if not isinstance(higher_is_better, bool):
        raise TypeError("Use only boolean (True or False) values")
            ...
            for i in range(len(adaptive)):
                ...
                if not isinstance(adaptive_value, (int, float, np.integer, np.floating)):
                    ...
                    if higher_is_better == True:
                        difference = adaptive_value-fixed_value
                        gains = (difference / abs(fixed_value)) * 100
                        adaptive_gains_array.append([seeds[i], gains])

                    elif higher_is_better == False:
                        difference = fixed_value-adaptive_value
                        gains = (difference / abs(fixed_value)) * 100
                        adaptive_gains_array.append([seeds[i], gains])
